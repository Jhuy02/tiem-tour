...
import OurBusService from '@/app/(main)/tours/[slug]/_components/transport-services/OurBusService'
    ...
    <div className='font-trip-sans flex flex-col space-y-[1.5rem] rounded-[1.5rem] border border-solid border-[#EDEDED] bg-white px-[1.75rem] py-[1.875rem]'>
      ...
      <div className='flex flex-col space-y-[1rem]'>
        ...
        <Tabs
          ...
          className='flex w-full flex-col gap-0 space-y-[0.75rem]'
        >
              ...
              TransportServiceList?.map((item, index) => {
                return (
                  <TabsTrigger
                    key={index}
                    value={item?.slug}
                    className='relative rounded-[1.25rem] bg-[#EBEBEB] p-0! shadow-none! before:absolute before:inset-0 before:z-0 before:rounded-[1.25rem] before:bg-[linear-gradient(180deg,#F4F5E6_0%,#B2DFDC_100%)] before:opacity-0 data-[state=active]:before:opacity-100'
                  >
